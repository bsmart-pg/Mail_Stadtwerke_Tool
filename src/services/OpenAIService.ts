...
class OpenAIService {
  ...
  async analyzeEmailText(subject: string, body: string): Promise<{
    ...
    try {
      const prompt = `Analysiere den folgenden E-Mail-Text sehr sorgfältig und extrahiere ALLE Kundennummern (falls vorhanden) und ordne die E-Mail ALLEN zutreffenden Kategorien zu:
      ...
      try {
        ...
        if (Array.isArray(result.allCategories)) {
          ...
          if (result.allCategories.length === 0) {
            result.allCategories = [result.category || 'Nicht definiert'];
          }
        } else {
          result.allCategories = [result.category || 'Nicht definiert'];
        }
        ...
        if (!validCategories.includes(result.category)) {
          result.category = result.allCategories.length > 0 ? result.allCategories[0] : 'Nicht definiert';
        }
        ...
        messages: [
          {
            ...
            content: [
              {
                type: "text",
                text: `Analysiere dieses Bild sehr sorgfältig. Extrahiere ALLE Kundennummern (falls vorhanden) und ordne das Bild ALLEN zutreffenden Kategorien zu:
...
Antworte NUR im Format: {"customerNumber": "erste gefundene Nummer oder null", "category": "hauptsächliche Kategorie", "allCustomerNumbers": ["array aller Kundennummern"], "allCategories": ["array aller Kategorien"]}`
  ...
  async analyzeText(prompt: string): Promise<string> {
    try {
      ...
      return completion.choices[0]?.message?.content || '{"customerNumber": null, "category": "Nicht definiert"}';
    } catch (error) {
